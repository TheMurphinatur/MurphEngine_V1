...
BulletCollider::BulletCollider(MurphEngine::IActor* pOwner)
    : DynamicCollider(pOwner)
{
    m_id = "BulletCollider";
}

void BulletCollider::HandleCollisionEvent(MurphEngine::Message collisionEvent, MurphEngine::IActor* pActor,
                                           MurphEngine::IActor* OtherActor)
{
    ...
    if (collisionEvent.GetID() == std::string("Collision") && pActor == this->m_pOwnerActor)
    {
        if (OtherActor->GetComponent("ShieldCollider") != nullptr ||
            OtherActor->GetComponent("EnemyShipCollider") != nullptr)
        {
...
void BulletCollider::Initialize()
{
...
void BulletCollider::Update(double deltaTime)
{
    if (m_pOwnerActor->GetY() < 0)
    {
        m_pOwnerActor->GetManager()->QueueRemoveActor(m_pOwnerActor->GetNumber());
        return;
    }
